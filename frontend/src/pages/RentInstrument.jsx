...
const RentInstrument = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(null);
      toast.success('Instrument rental confirmed!');
      ...
      const message = err.response?.data?.message || 'Failed to create rental. Please try again.';
      setError(message);
      toast.error(message);
                ...
                {instruments.map((instrument) => (
                  ...
                  >
                    {instrument.images && instrument.images.length > 0 ? (
                      <img 
                        src={getImageUrl(instrument.images[0])}
                        alt={instrument.name} 
